...
.dsh-about-release-body { overflow: hidden; }
.dsh-about-release-body-collapsed { display: none; }
...
.dsh-about-source { margin-top: 2px; border-top: 1px solid rgba(128, 128, 128, 0.22); padding-top: 14px; }
.dsh-about-source-head { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; flex-wrap: wrap; }
.dsh-about-source-title { font-size: 14.5px; font-weight: 700; }
.dsh-about-source-hint { font-size: 12.5px; line-height: 1.6; color: var(--dsw-alias-label-tertiary, #7a7a7a); margin-bottom: 8px; }
.dsh-about-source-options { display: flex; flex-direction: column; gap: 2px; }
.dsh-about-source-opt { display: flex; align-items: center; gap: 8px; padding: 6px 4px; border-radius: 6px; cursor: pointer; font-size: 13.5px; }
.dsh-about-source-opt:hover { background: rgba(128, 128, 128, 0.08); }
.dsh-about-source-opt input { margin: 0; accent-color: var(--dsw-static-blue-400, #0a66c2); }
.dsh-about-source-opt label { cursor: pointer; display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; }
.dsh-about-source-opt-code { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; color: var(--dsw-alias-label-tertiary, #7a7a7a); overflow-wrap: anywhere; }
...
.dsh-about-source-actions { display: flex; gap: 8px; margin-top: 6px; flex-wrap: wrap; }
.dsh-about-source-current { font-size: 12px; color: var(--dsw-alias-state-success-primary, #1a7f37); }
...
@keyframes dsh-about-success-pop { from { transform: scale(0.9); opacity: 0; } to { transform: scale(1); opacity: 1; } }`;
		...
		function AboutSection() {
			...
			const [pinging, setPinging] = useState(false); // 正在检测（true=任意方式）
			...
			const switchSource = (sourceId) => {
				...
				fetch(ENDPOINT + "/source", {
					...
					.then((data) => {
						...
						setSourceState((prev) => (prev ? { ...prev, current: data.current, resolvedHost: data.resolvedHost } : prev));
						setSourceError(null);
					})
			...
			/** 延迟检测：mode="one" 只测当前选中（逐个），mode="all" 并发测全部（一起）。 */
			const pingSources = (mode) => {
				if (pinging) return;
				setPinging(true);
				const body = mode === "all" ? { sources: "all" } : { source: sourceState && sourceState.current };
				fetch(ENDPOINT + "/ping", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					...
					.then((data) => {
						...
						if (data.error && data.source === undefined && data.results === undefined) {
							setSourceError(String(data.error));
							return;
						}
						if (mode === "all" && data.results) setPings(data.results);
						else if (data.source) setPings({ [data.source]: { ok: data.ok === true, ms: data.ms, error: data.error } });
						setSourceError(null);
					})
					.catch((error) => setSourceError(String(error && error.message)))
				...
				describeError !== null
					...
					h("div", { className: "dsh-about-source" },
						h("div", { className: "dsh-about-source-head" },
							...
							sourceState !== null
								? h("span", { className: "dsh-about-source-current" }, "当前：", sourceState.sources.find((s) => s.id === sourceState.current)?.label || sourceState.current)
								: null
						),
						h("div", { className: "dsh-about-source-hint" }, "检查更新与一键更新所用的 npm registry。国内镜像（npmmirror）为官方源的缓存代理，同步存在轻微延迟；「本地配置」跟随 npm config get registry。"),
						sourceError !== null
							? h("div", { className: "dsh-about-status dsh-about-status-err" }, String(sourceError))
							: null,
						sourceState === null
							? h("div", { className: "dsh-about-muted" }, "正在获取更新源…")
							: h("div", { className: "dsh-about-source-options" },
								sourceState.sources.map((s) => {
									const ping = pings ? pings[s.id] : null;
									const checked = s.id === sourceState.current;
									return h("div", { className: "dsh-about-source-opt", key: s.id },
										h("label", null,
											h("input", {
												type: "radio",
												name: "dsh-about-source",
												value: s.id,
												checked,
												disabled: savingSource,
												onChange: () => switchSource(s.id)
											}),
											h("span", null, s.label),
											h("span", { className: "dsh-about-source-opt-code" }, s.host)
										),
										ping === null
											? h("span", { className: "dsh-about-source-lat dsh-about-source-lat-unknown" }, "未检测")
											: ping.ok
												? h("span", { className: "dsh-about-source-lat dsh-about-source-lat-ok" }, ping.ms + " ms")
												: h("span", { className: "dsh-about-source-lat dsh-about-source-lat-err", title: String(ping.error || "连接失败") }, "不可达")
									);
								})
							),
						h("div", { className: "dsh-about-source-actions" },
							h("button", { className: "dsh-about-btn", type: "button", onClick: () => pingSources("one"), disabled: pinging || sourceState === null },
								pinging ? "检测中…" : "逐个检测（当前源）"
							),
							h("button", { className: "dsh-about-btn", type: "button", onClick: () => pingSources("all"), disabled: pinging || sourceState === null },
								pinging ? "检测中…" : "一起检测（全部源）"
							),
							pings !== null && !pinging
								? h("button", { className: "dsh-about-btn dsh-about-btn-refresh", type: "button", onClick: () => setPings(null) }, "清除结果")
								: null
						)
					),
							...
							: releases.releases.length === 0
								...
								: h("div", null, releases.releases.map((release, index) => {
									...
									return h("div", { className: "dsh-about-release", key: release.version },
										...
										hasBody ? h("div", { className: "dsh-about-release-body" + (expanded ? "" : " dsh-about-release-body-collapsed") }, renderReleaseBody(release.body)) : null
									);
								}))
				),
				successShow
