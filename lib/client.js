...
.dsh-about-source-select-arrow { flex: none; font-size: 11px; color: var(--dsw-alias-label-tertiary, #7a7a7a); transition: transform 0.15s ease; }
...
@keyframes dsh-about-success-pop { from { transform: scale(0.9); opacity: 0; } to { transform: scale(1); opacity: 1; } }`;
		...
		function AboutSection() {
			...
			const [pinging, setPinging] = useState(false); // 正在检测（任意源）
			...
			const pingSource = (sourceId) => {
				...
				fetch(ENDPOINT + "/ping", {
					...
					.then((data) => {
						...
						if (data.source) setPings({ ...(pings || {}), [data.source]: { ok: data.ok === true, ms: data.ms, error: data.error } });
						setSourceError(null);
					})
					.catch((error) => setSourceError(String(error && error.message)))
					...
					h("div", { className: "dsh-about-source" },
						h("div", { className: "dsh-about-source-head" },
							...
							sourceState === null
								...
								: h("div", { className: "dsh-about-source-select", role: "combobox", "aria-expanded": sourceOpen, tabIndex: 0, onClick: () => setSourceOpen(!sourceOpen), onKeyDown: (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setSourceOpen(!sourceOpen); } } },
