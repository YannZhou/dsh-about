...
// 每日首次打开自动拉取一次（缓存对象记录日期），其余时间仅点「刷新」才拉取。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 已拉取的版本记录内容缓存：{ fetchedOn: "YYYY-MM-DD", releases: [...] }。
		 * 每日门控仅由 fetchedOn 字段承担——没有单独的日期键。 */
		const CACHE_KEY = "dsh-about:releases-cache";

		/** 本地时区的今天，形如 2026-08-20。 */
		function todayStr() {
			const d = new Date();
		...
		function readReleasesCache() {
			try {
				...
				if (parsed !== null && typeof parsed === "object" && Array.isArray(parsed.releases)) {
					// 条目级形状规整：缓存可能来自旧版本格式或外部污染，
					// 任何字段缺失/类型不符都归一为安全值，避免渲染阶段抛错
					...
					return { fetchedOn: typeof parsed.fetchedOn === "string" ? parsed.fetchedOn : "", releases };
				}
				return null;
			} catch {
				return null;
		...
		/** 保存版本记录缓存（含拉取日期）。 */
		function writeReleasesCache(fetchedOn, releases) {
			try {
				window.localStorage.setItem(CACHE_KEY, JSON.stringify({ fetchedOn, releases }));
			} catch {
				/* 配额/私密模式失败时忽略：仅本次会话内存中展示 */
			}
		}
...
.dsh-about-muted { color: var(--dsw-alias-label-tertiary, #7a7a7a); font-size: 12.5px; }
...
.dsh-about-dialog { width: min(460px, 94vw); max-height: 84vh; display: flex; flex-direction: column; gap: 12px; background: var(--dsw-alias-bg-layer-3, #ffffff); color: var(--dsw-alias-label-primary, #1f1f1f); border: 1px solid rgba(128, 128, 128, 0.35); border-radius: 10px; padding: 18px; box-shadow: 0 12px 40px rgba(0, 0, 0, 0.25); }
.dsh-about-dialog-title { font-size: 15.5px; font-weight: 700; }
.dsh-about-versions { display: flex; flex-direction: column; gap: 6px; overflow: auto; }
.dsh-about-version { display: flex; align-items: center; gap: 8px; padding: 9px 12px; border: 1px solid rgba(128, 128, 128, 0.28); border-radius: 8px; cursor: pointer; font-size: 13.5px; }
...
.dsh-about-dialog-actions { display: flex; justify-content: flex-end; gap: 10px; }`;
		...
		function AboutSection() {
			...
			useEffect(() => {
				// 每日门控 + 内容缓存（门控仅由缓存对象的 fetchedOn 字段承担）：
				//  - 同日且已有缓存 → 直接展示保存的内容，不请求网络
				//  - 无缓存或日期不同 → 自动拉取一次，成功后写入缓存
				//  - 拉取失败但存在旧缓存 → 展示旧缓存并提示
				const today = todayStr();
				const cache = readReleasesCache();
				if (cache !== null && cache.fetchedOn === today) {
					setReleases({ ok: true, releases: cache.releases });
					return;
				}
				let alive = true;
				fetch(ENDPOINT + "/releases")
					...
					.then((data) => {
						...
						if (data.ok === true) {
							...
							writeReleasesCache(today, releases);
							setReleases({ ok: true, releases });
						} else {
							// 拉取失败但有旧缓存时，至少展示保存过的内容
							if (cache !== null) setReleases({ ok: true, releases: cache.releases, stale: true });
					...
					.catch((error) => {
						if (!alive) return;
						if (cache !== null) setReleases({ ok: true, releases: cache.releases, stale: true });
						else setReleases({ ok: false, error: String(error && error.message), releases: [] });
					});
				return () => {
					alive = false;
				};
			...
			/** 刷新失败时的兜底展示：保留已展示的内容，仅叠加 stale 提示；无可保留内容时才显示错误。 */
			const keepShownOnFailure = (error) => {
				setReleases((prev) =>
					prev !== null && Array.isArray(prev.releases) && prev.releases.length > 0
						? { ok: true, releases: prev.releases, stale: true }
			...
			const onRefresh = () => {
				...
				fetch(ENDPOINT + "/releases")
					.then((res) => res.json())
					.then((data) => {
						if (data && data.ok === true && Array.isArray(data.releases)) {
							writeReleasesCache(todayStr(), data.releases);
							setReleases({ ok: true, releases: data.releases });
						} else {
							keepShownOnFailure(String((data && data.error) || "刷新响应异常"));
						}
						setRefreshing(false);
					})
					.catch((error) => {
						keepShownOnFailure(String(error && error.message));
						setRefreshing(false);
					});
			};
			...
			return h("div", { className: "dsh-about" },
				...
				h("div", { className: "dsh-about-update" },
					...
					snapshot !== null && snapshot.ok && snapshot.updateAvailable && !pickerOpen && !restarting
						? h("div", { className: "dsh-about-update-line" },
				...
				h("div", { className: "dsh-about-releases" },
					...
					releases !== null && releases.stale === true
						? h("div", { className: "dsh-about-status dsh-about-status-warn" }, "网络获取失败，当前展示的是本地保存的内容，可点「刷新」重试")
						: null,
					releases === null
