...
function scheduleAutoRestart(delayMs = 2500, portOverride) {
	...
	try {
		...
		const home = os.homedir();
		// systemd-run 的 --setenv= 会覆盖子进程的全部环境，故 PATH 必须保留真实值，
		// 否则 nvm / 自定义 prefix 等常见 node 安装位置的 `#!/usr/bin/env node`
		// 会在 systemd-run 的一次看护里找不到 node，导致更新后无人自动重启。
		// 保留真实 PATH，再把 npm 全局 bin 兜底追加到末尾。
		const preservedPath = [process.env.PATH, path.join(home, ".npm-global", "bin")]
			.filter(Boolean)
			.join(":");
