...
/** 把选中源落盘（目录由版本记录缓存逻辑负责创建，这里容错）。 */
function writeSourceState(sourceId) {
	try {
		fsp.mkdir(path.dirname(SOURCE_FILE), { recursive: true }).then(() =>
			fsp.writeFile(SOURCE_FILE, JSON.stringify({ source: sourceId }, null, 2), "utf8")
		).catch(() => {});
