...
function writeSourceState(sourceId) {
	try {
		fsp.mkdirSync(path.dirname(SOURCE_FILE), { recursive: true });
		fsp.writeFileSync(SOURCE_FILE, JSON.stringify({ source: sourceId }, null, 2), "utf8");
...
function resolveWatchdog() {
	...
	const candidates = [
		...
		{ path: `${home}/.local/bin/dsh-watchdog`, needsExec: true }
	];
	for (const candidate of candidates) {
		try {
...
function scheduleAutoRestart(delayMs = 2500, portOverride) {
	...
	try {
		...
		if (sysRun !== null) {
			...
			// bash 版（无 systemd-run 的 Linux 兜底）：detached bash -c
			restarter = spawn(
				"/usr/bin/bash",
				["-c", `exec "${watchdog.path}" once`],
				{
					detached: true,
					stdio: "ignore",
